...
export abstract class ProjectMemberRepository {
  ...
  // 문자열을 포함하는 멤버 목록 조회
  abstract findMembersByString(projectPk: number, searchString: string): Promise<ProjectMember[]>; 
  ...
  abstract addMember(manager: EntityManager, projectPk: number, userPk: number, lastConnectedChannel?: number): Promise<void>;
}
