...
export class ProjectCreationService {
  ...
  async createProject(
    ...
    if (createProjectDto.serverUrl) {
      server = await this.serverRepository.findOne({
        where: {
          serverUrl: createProjectDto.serverUrl,
          isDeletedServer: false,
        },
      });
      if (!server) {
        throw new NotFoundException(
          `서버 URL ${createProjectDto.serverUrl}을 찾을 수 없습니다`,
        );
      }
      ...
      if (!server) {
        throw new NotFoundException(
          `서버 ID ${createProjectDto.serverPk}를 찾을 수 없습니다`,
        );
      }
    } else {
      throw new NotFoundException(
        'serverUrl 또는 serverPk 중 하나는 반드시 제공되어야 합니다',
      );
    }
  ...
  async getProjectsByServerForUser(
    serverUrl: string,
    userPk: number,
    ...
    return projectMembers
      .filter(
        (member) =>
          member.project.serverPk === server.serverPk &&
          !member.project.isDeletedProject,
      )
      .map((member) => ({
  ...
  async getProjectsByServer(serverUrl: string): Promise<ProjectResponseDto[]> {
    ...
    return projects.map((project) => {
      const admin = project.projectMembers.find(
        (member) => member.projectRole === 'admin',
      );
