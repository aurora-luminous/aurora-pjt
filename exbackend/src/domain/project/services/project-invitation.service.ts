...
export class ProjectInvitationService {
  ...
  private async hasProjectManagePermission(
    ...
    if (
      projectMember &&
      MemberRoleUtils.hasAdminPermission(projectMember.projectRole)
    ) {
      return true;
    }
    ...
    return serverMember
      ? ServerRoleUtils.hasProjectCreatePermission(serverMember.serverRole)
      : false;
  }

  // 이메일로 프로젝트에 사용자 초대 (직접 추가)
  async inviteUserToProject(
    inviteDto: InviteToProjectDto,
    ...
    if (!project) {
      throw new NotFoundException(
        `프로젝트 ID ${inviteDto.projectPk}를 찾을 수 없습니다`,
      );
    }

    // 2. 초대하려는 사용자 존재 확인 (이메일로)
    const targetUser = await this.userService.getUserByEmail(
      inviteDto.userEmail,
    );
    ...
    if (!hasPermission) {
      throw new ForbiddenException(
        '프로젝트 관리자 또는 서버 관리자/소유자/프로젝트 매니저만 사용자를 초대할 수 있습니다',
      );
    }
    ...
    if (existingMember) {
      if (existingMember.pStatus === 'Active') {
        ...
        await this.addMemberToPublicChannels(
          inviteDto.projectPk,
          targetUser.userPk,
        );
  ...
  // 여러 사용자를 프로젝트에 일괄 초대
  async bulkInviteUsersToProject(
    bulkInviteDto: BulkInviteToProjectDto,
  ): Promise<void> {
    ...
    if (!project) {
      throw new NotFoundException(
        `프로젝트 ID ${bulkInviteDto.projectPk}를 찾을 수 없습니다`,
      );
    }
  ...
  async getProjectMembers(
    ...
    if (!requestMember && !hasPermission) {
      throw new ForbiddenException(
        '프로젝트 멤버 또는 서버 관리자/소유자/프로젝트 매니저만 멤버 목록을 볼 수 있습니다',
      );
    }
  ...
  async removeUserFromProject(removeDto: RemoveFromProjectDto): Promise<void> {
    ...
    if (!project) {
      throw new NotFoundException(
        `프로젝트 ID ${removeDto.projectPk}를 찾을 수 없습니다`,
      );
    }
    ...
    if (!hasPermission) {
      throw new ForbiddenException(
        '프로젝트 관리자 또는 서버 관리자/소유자/프로젝트 매니저만 멤버를 제거할 수 있습니다',
      );
    }
    ...
    await this._updateProjectMemberStatus(
      targetMember.projectMemberPk,
      'Inactive',
    );
  ...
  async banUserFromProject(
    ...
    if (!hasPermission) {
      throw new ForbiddenException(
        '프로젝트 관리자 또는 서버 관리자/소유자/프로젝트 매니저만 멤버를 차단할 수 있습니다',
      );
    }
  ...
  async unbanUserFromProject(
    ...
    if (!hasPermission) {
      throw new ForbiddenException(
        '프로젝트 관리자 또는 서버 관리자/소유자/프로젝트 매니저만 멤버의 차단을 해제할 수 있습니다',
      );
    }

    // 4. 상태를 Active로 복구
    bannedMember.pStatus = 'Active';
    const unbannedMember =
      await this.projectMemberRepository.save(bannedMember);
  ...
  async leaveProject(
    ...
    if (
      ...
      if (otherActiveAdmins.length === 0) {
        ...
        const membersToKickCount = allMembers.filter(
          (member) =>
            member.userPk !== userPk && member.projectRole === 'member',
        ).length;
  ...
  private async _updateProjectMemberStatus(
    ...
    if (!projectMember) {
      throw new NotFoundException(
        `ProjectMember with PK ${projectMemberPk} not found.`,
      );
    }
