...
export class ServerController {
  ...
  @ApiResponse({
    ...
    type: Object,
    schema: { example: { inviteHash: 'a1b2c3d4e5f6' } },
  }) // Swagger 응답 업데이트
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth('access-token')
  ...
  async bulkUpdateMemberRoles(
    ...
    processed: number;
    failed: Array<{ userEmail: string; reason: string }>;
  }> {
    ...
    // 권한 일괄 변경 실행
    const result =
      await this.serverMemberManagementService.bulkUpdateMemberRoles(
        server.serverPk,
        updateDto.changes,
        ownerUserPk,
      );
  ...
  @ApiResponse({
    status: 403,
    description: '서버 소유자는 서버를 나갈 수 없음',
  })
  @ApiResponse({
    status: 404,
    description: '서버 또는 활성 멤버를 찾을 수 없음',
  })
  async leaveServer(
    @Param('serverUrl') serverUrl: string,
    @CurrentUser() user: User,
