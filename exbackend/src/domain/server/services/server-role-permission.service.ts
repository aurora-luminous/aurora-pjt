...
export class ServerRolePermissionService {
  ...
  // 서버 권한 조회
  async getServerPermissions(serverUrl: string, requestUserPk: number): Promise<GetServerPermissionsResponseDto> {
    // 1. 서버 존재 확인
    ...
    const requestMember = await this.serverMemberRepository.findOne({
      where: { serverPk: server.serverPk, userPk: requestUserPk, sStatus: 'Active' }
    });
  ...
  async updateRolePermission(serverUrl: string, updateDto: Omit<UpdateRolePermissionDto, 'serverPk'>, requestUserPk: number): Promise<ServerRolePermissionResponseDto> {
    // 1. 서버 존재 확인
    const server = await this.serverRepository.findOne({
      where: { serverUrl: serverUrl, isDeletedServer: false }
    });
