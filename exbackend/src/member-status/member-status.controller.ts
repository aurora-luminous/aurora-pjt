...
export class MemberStatusController {
  ...
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: '마지막 접속 채널 정보를 찾을 수 없음' })
  async getLastConnectedChannel(
    @CurrentUser() user: User,
  ): Promise<LastChannelInfoDto | null> {
    const lastChannelInfo = await this.projectMemberUpdateService.getLastConnectedChannelInfo(user.userPk);
    if (!lastChannelInfo) {
