...
import { useAdminPermission } from "@/app/(servers)/[server_id]/admin/hooks/useAdmin";
...
const ProjectsPage = () => {
    ...
    return (
      <div className="flex h-full bg-gray-900 items-center justify-center">
        <div className={`text-white text-center ${isMobile ? "px-4" : "px-0"}`}>
          ...
          <div
            className={`border-2 border-white border-t-transparent rounded-full animate-spin mx-auto ${isMobile ? "w-6 h-6" : "w-8 h-8"}`}
          />
        </div>
      </div>
    );
  ...
  if (!isAdmin) {
    return (
      <div className="flex h-full bg-gray-900 items-center justify-center">
        <div
          className={`text-center bg-red-900/20 border border-red-600 rounded-lg ${isMobile ? "p-6 mx-4 max-w-sm" : "p-8 max-w-md"}`}
        >
          <div
            className={`text-red-400 mb-4 ${isMobile ? "text-4xl" : "text-6xl"}`}
          >
            🚫
          </div>
          <h1
            className={`text-white font-bold mb-2 ${isMobile ? "text-xl" : "text-2xl"}`}
          >
            접근 권한이 없습니다
          </h1>
          <p
            className={`text-gray-300 mb-4 ${isMobile ? "text-sm" : "text-base"}`}
          >
  ...
  return (
    <div className={`h-full overflow-auto ${isMobile ? "p-4" : "p-6"}`}>
      ...
      {/* 안내 메시지 */}
      <div
        className={`mb-6 bg-indigo-900/20 border border-indigo-500/30 rounded-lg ${isMobile ? "p-3" : "p-4"}`}
      >
        <div
          className={`flex items-start ${isMobile ? "space-x-2" : "space-x-3"}`}
        >
          <span
            className={`text-indigo-400 ${isMobile ? "text-base" : "text-lg"}`}
          >
            💡
          </span>
          <div>
            <h3
              className={`text-indigo-200 font-medium mb-1 ${isMobile ? "text-sm" : "text-base"}`}
            >
              프로젝트 안내
            </h3>
            <p
              className={`text-indigo-300 ${isMobile ? "text-xs" : "text-sm"}`}
            >
              프로젝트를 삭제하면 해당 프로젝트의 모든 채널과 데이터가 함께
              삭제됩니다.
            </p>
          </div>
        </div>
