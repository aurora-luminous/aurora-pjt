...
export const VoiceParticipantCard = ({
  ...
  const iconPosition = isCompact ? "bottom-2 left-2" : "bottom-4 left-4";
  ...
  return (
    ...
    >
      ...
      {/* 사용자 이름 */}
      <span
        className={`${textSize} font-medium text-white ${
          isCompact ? "mb-1" : "mb-2"
        }`}
      >
      ...
      {/* 상태 표시 */}
      <div
        className={`absolute ${iconPosition} flex gap-${isCompact ? "1" : "2"}`}
      >
        {!participant.isMicOn && (
          <div
