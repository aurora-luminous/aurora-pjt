...
export class MediasoupService implements OnModuleInit, OnModuleDestroy {
  ...
  // mediasoup 설정
  private getMediasoupConfig() {
    return {
      worker: {
        rtcMinPort: this.configService.get<number>('MEDIASOUP_RTC_MIN_PORT', 10000),
        rtcMaxPort: this.configService.get<number>('MEDIASOUP_RTC_MAX_PORT', 10100),
      ...
      webRtcTransport: {
        listenIps: [
          {
            ip: this.configService.get<string>('MEDIASOUP_LISTEN_IPS', '0.0.0.0'),
  ...
  private async createWorkers() {
    ...
    const config = this.getMediasoupConfig();
  ...
  getWebRtcTransportOptions() {
    return this.getMediasoupConfig().webRtcTransport;
  }
